...
namespace DataminingProject.Algorithms
{
    public class AprioriClose
    {
        ...
        Dictionary<List<string>, int> previousCandidateSupportCounts = null;
        ...
        public AprioriClose(double minSupport)
        {
        ...
        private List<List<string>> GenerateItemSet(int index)
        {
            ...
            //save the current candidateSupportCounts before resetting
            previousCandidateSupportCounts = candidateSupportCounts;
            ...
            if (File.Exists(this.InputDataFile))
            {
                using (FileStream fs = new FileStream(this.InputDataFile, FileMode.Open, FileAccess.Read))
                {
                    using (StreamReader sr = new StreamReader(fs))
                    {
                        ...
                        foreach (var previousPair in previousCandidateSupportCounts)
                        {
                            bool closed = true;
                            
                            foreach (KeyValuePair<List<string>,int> currentPair in candidateSupportCounts)
                            {
                                if (previousPair.Value == currentPair.Value)
                                {
                                    if (string.Join(",", currentPair.Key).Contains(string.Join(",", previousPair.Key)))
                                    {
                                        closed = false;
                                        break;
                                    }
